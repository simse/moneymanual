...
function plan2InterestRate(
	salary: number,
	lower: number,
	upper: number,
): number {
	if (salary <= lower) return PLAN_2_LOWER_INTEREST;
	if (salary >= upper) return PLAN_2_UPPER_INTEREST;
...
export function computeStudentLoanRepayment(
