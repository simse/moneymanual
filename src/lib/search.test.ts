...
const completion = (phrases: string[], id = "one") => ({
  document: { id, searchPhrases: phrases },
  highlights: [
    {
      field: "searchPhrases",
...
beforeEach(() => {
  ...
  index.add({ id: "economy/inflation", title: "Inflation" });
...
it("maps Typesense results and encodes the query", async () => {
  fetchMock.mockResolvedValue(response([article()]));

  expect((await search("inflation & tax+", request)).results).toEqual([
  ...
  expect(parsed.searchParams.has("filter_by")).toBe(false);
...
it("retrieves every result page", async () => {
  ...
  const result = await search("inflation", request);
...
it.each(["search", "autocomplete"] as const)(
  "keeps valid empty %s results",
  async (kind) => {
    fetchMock.mockResolvedValue(response([]));

    expect(
      await (kind === "search" ? search : autocomplete)("none", request),
...
it("maps phrase indices and tokens, deduplicates and limits suggestions", async () => {
  ...
  const { results } = await autocomplete("infla", request);
...
it("uses only matching phrase indices", async () => {
  ...
  expect((await autocomplete("infla", request)).results).toEqual([
...
describe("fallback", () => {
  ...
  it("enforces one deadline across pages", async () => {
    ...
    const result = search("inflation", request);
...
it("rejects missing queries before contacting either provider", async () => {
  const result = await searchRoute("search")({
    url: new URL("https://example.com/api/search"),
    request,
  } as APIContext);
...
it.each([0, 7, undefined, -1, "3"])(
  "handles optional Typesense timing %s",
  async (timing) => {
    fetchMock.mockResolvedValue(
      Response.json({ hits: [], found: 0, search_time_ms: timing }),
    );

    const result = await search("none", request);
    expect(result.provider).toBe("typesense");
...
it("includes provider and timing in autocomplete API responses", async () => {
  ...
  const result = await searchRoute("autocomplete")({
    url: new URL(request.url),
    request,
  } as APIContext);
...
it("shows query time including zero and omits it when absent", async () => {
  ...
  expect(copy.resultsFound(2, "inflation", 0)).toBe(
    'Found 2 results for "inflation" in 0 ms',
  );

  expect(copy.resultsFound(2, "inflation", 7)).toBe(
    'Found 2 results for "inflation" in 7 ms',
  );

  expect(copy.resultsFound(0, "inflation")).toBe(
