...
const string = (value: unknown): string => {
  if (typeof value !== "string") {
    throw new Error("Invalid Typesense field");
  }
...
const strings = (value: unknown): string[] => {
  if (!Array.isArray(value)) {
    throw new Error("Invalid Typesense array");
  }

  return value.map(string);
};

const searchResult = (document: Record<string, unknown>): SearchResult => {
...
const suggestions = (
  ...
  return highlight.indices.map((index, position) => {
    ...
    if (!matchedTokens.length) {
      throw new Error("Missing matched tokens");
    }
...
export const searchTypesense: {
  ...
  SearchResponse<SearchResult> | SearchResponse<AutocompleteResult>
> => {
  ...
  url.search = new URLSearchParams({
    ...
    per_page: "100",
    include_fields:
      kind === "search" ? "id,title,description,url" : "id,searchPhrases",
  ...
  const collect = async () => {
    ...
    for (let page = 1; ; page++) {
        ...
        if (kind === "search") {
          ...
          for (const result of suggestions(hit, document)) {
            ...
            if (!completions.has(identity)) {
              completions.set(identity, result);
            }
          }
        }
      }
      ...
      if (ids.size > total) {
        throw new Error("Invalid Typesense count");
      }
