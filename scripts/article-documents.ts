...
async function* files(directory: string, prefix = ""): AsyncGenerator<string> {
	for (const entry of await readdir(directory, { withFileTypes: true })) {
		const relative = prefix + entry.name;
		if (entry.isDirectory())
			yield* files(join(directory, entry.name), `${relative}/`);
		else if (entry.isFile() && /\.(md|mdx)$/.test(entry.name)) yield relative;
	}
}
...
export async function readArticles(
	...
	for await (const file of files(directory)) {
		if (file.split("/")[1] !== "en-gb") continue;
		try {
	...
	if (!documents.length)
		throw new Error(
			"No English articles found; refusing to sync an empty source set",
		);
