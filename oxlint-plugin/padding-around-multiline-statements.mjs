// Source: https://github.com/zoonk/zoonk/blob/main/packages/oxlint-plugin/rules/padding-around-multiline-statements.js
 ...
 */
const isPlainWhitespaceGap = ({
  sourceText,
  previousStatement,
  nextStatement,
}) => {
  const gap = sourceText.slice(
    previousStatement.range[1],
 ...
 */
const hasBlankLineBetween = ({ previousStatement, nextStatement }) => {
 ...
 */
const getStatementIndentation = ({ sourceText, statement }) => {
 ...
 */
const shouldCheckStatementPair = ({
  sourceText,
  previousStatement,
  nextStatement,
}) => {
  ...
  if (
    isModuleBoundaryStatement(previousStatement) ||
    isModuleBoundaryStatement(nextStatement)
  ) {
    return false;
  }

  if (
    !isMultilineStatement(previousStatement) &&
    !isMultilineStatement(nextStatement)
  ) {
    return false;
  }
...
const getReplacementGap = ({
  ...
  const indentation = getStatementIndentation({
    sourceText,
